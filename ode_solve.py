...
def adaptive_RK(fun, y0, args, dt, tableau, tol, order):
    ...
    q = min(0.8 * (maxerr / E) ** (1 / (order + 1)), 4.0)
    dt_new = dt * q
    if E > maxerr:
        return adaptive_RK(fun, y0, args, dt_new, tableau, tol, order)
